...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        lifecycleScope.launch {
            ...
            val setupCompleted = setupCompletedDocRef.get(Source.CACHE).runCatching { await() }.fold(onSuccess = { it.exists() }, onFailure = { false })

            val testState: RunState.Test = if(setupCompleted) {
                ...
                repeat(DOCUMENT_CREATE_AND_DELETE_COUNT) {
                    ...
                    if (batchSize == MAX_FIRESTORE_BATCH_SIZE
                        ...
                        setupState = setupState.run {
                            copy(documentCreateCount=documentCreateCount+batchSize)
                        }
                        runState.value = setupState

                        deleteBatch.commit().await()
                        setupState = setupState.run {
                            copy(documentDeleteCount=documentDeleteCount+batchSize)
                        }
                        runState.value = setupState
        ...
        setContent {
            FirebaseLocalSampleTheme {
                Column(
                    ...
                    val setupState: RunState.Setup? = when (val state = runState.value) {
                    ...
                    if (setupState !== null) {
                        when (runState.value) {
                            ...
                            is RunState.Test, is RunState.Done -> Text("Setup completed.")
                        }
                    ...
                    when (runState.value) {
                        RunState.NotStarted, is RunState.Setup -> {}
                    ...
                    when (runState.value) {
                        RunState.NotStarted, is RunState.Setup -> {}
                        is RunState.Test, is RunState.Done -> {
                            Text(
